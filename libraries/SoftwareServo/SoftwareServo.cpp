...
uint8_t SoftwareServo::attach(int pinArg)	//attach servo object
{
    pin = pinArg;
    angle = NO_ANGLE;
    pulse0 = 0;
...
void SoftwareServo::detach()			//detach servo object
{
...
void SoftwareServo::write(int angleArg)		//set desired position (requieres some time to achieve it)
{
...
uint8_t SoftwareServo::read()			//returns current state angle
{
    return angle;
}

uint8_t SoftwareServo::attached()		//check if object is attached or not
{
    for ( SoftwareServo *p = first; p != 0; p = p->next ) {
	if ( p == this) return 1;
    }
